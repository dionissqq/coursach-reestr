...
export default function EditAgency(props) {
    ...
    function getFormProps() {
            ...
            inputs : [
                ...
                },
                {
                    ...
                    props : {
                        ...
                        selected : new Date (entity.data.submission_date ? moment(entity.data.register_registration_date, 'DD/MM/YYYY') : moment()),
                    }
                },
                {
                ...
                },
                {
                    ...
                    props : {
                        ...
                        selected: new Date (entity.data.gov_reg_date ? moment(entity.data.gov_reg_date, 'DD/MM/YYYY') : moment()),
                    }
                },
                {
                    type : Form.InputType.Text,
            ...
            props.inputs = props.inputs.concat([
                ...
                },
                {
                    ...
                    props : {
                        ...
                        defaultValue : entity.data.duplicate_issued,
                    }
                }
            ])
        }
        props.inputs.push({
            type : Form.InputType.Checkbox,
            props : {
                label : "Інформаційне агенство припиноло діяльтність",
                id : "terminated",
                checked : terminated,
                onClick : () => {
            ...
            props.inputs = props.inputs.concat([
                {
                    ...
                    props : {
                        ...
                        label : "Дата припинення діяльності інформагенства",
                        selected : new Date (entity.data.termination_date ? moment(entity.data.termination_date, 'DD/MM/YYYY') : moment()),
                    }
                },
                {
                    type : Form.InputType.Text,
                    props : {
                        required : true,
                        type : "text",
                        name : "termination_doc_name",
                        maxLength : "128",
                        placeholder : "Назва документу, що є підставою для припинення діяльності інформагенства",
                        defaultValue : entity.data.termination_doc_name,
                    }
                },
                {
                    type : Form.InputType.Text,
                    props : {
                        required : true,
                        type : "number",
                        name : "termination_doc_number",
                        maxLength : "16",
                        placeholder : "Номер документу, що є підставою для припинення діяльності інформагенства",
                        defaultValue : entity.data.termination_doc_number
                    }
                },
                {
                    type : Form.InputType.Date,
                    props : {
                        required : true,
                        name : "termination_doc_date",
                        selected : new Date (entity.data.termination_doc_date ? moment(entity.data.termination_doc_date, 'DD/MM/YYYY') : moment()),
                        label : "Дата документу, що є підставою для припинення діяльності інформагенства",
                    }
                },
                {
                    type : Form.InputType.Text,
                    props : {
                        required : true,
                        type : "text",
                        name : "termination_doc_issued",
                        maxLength : "128",
                        placeholder : "Ким видано документ, що є підставою для припинення діяльності інформагенства",
                        defaultValue : entity.data.termination_doc_issued,
                    }
                },
                {
                    type : Form.InputType.Text,
                    props : {
                        ...
                        maxLength : "256",
                        placeholder : "Причина припинення діяльності інформагенства",
                        defaultValue : entity.data.termination_reason,
                    }
                }
            ])
        }
    ...
    async function onSubmit(form) {
        ...
        if (new Date(moment(formData.get("changes_doc_date"), 'DD/MM/YYYY')) > Date.now()){
            setInfoFormData({
                message: "Дата документу, що є підставою для внесення змін, не може бути майбутньою!",
                redirectURI: ""
            })
            setSubmitButtonFaded(false);
            return
        ...
        const ddd = formData.get("duplicate_doc_date");
        if (ddd && new Date(moment(ddd, 'DD/MM/YYYY')) > Date.now()){
            setInfoFormData({
                message: "Дата документу, що є підставою для внесення змін, не може бути майбутньою!",
                redirectURI: ""
            })
        ...
        const dd = formData.get("duplicate_date");
        if (dd && new Date(moment(dd, 'DD/MM/YYYY')) > Date.now()){
            setInfoFormData({
                message: "Дата видачі дупліката свідоцтва про реєстрацію інформагенства не може бути майбутньою!",
                redirectURI: ""
            })
            setSubmitButtonFaded(false);
            return
    ...
    if (entity.isLoaded) {
        ...
        Agency.getById(id)
            .then(result => {
                ...
                setDuplicateExists(result.data.duplicate_doc_date ? true : false);
                setTerminated(result.data.termination_date ? true : false)
                setWarning(Messages.Empty);
            })
        // {
        //     setEntity({
        ...
        //         data : {
        //             id: 1,
        //             submission_date:"31/10/1989",
        //             founders: "aa\naa",
        //             name:"",
        //             doc_registration_unit:"",
        //             register_registration_date:"24/09/2017",
        //             type: 4565,
        //             languages:"",
        //             distribution_scope:5552,
        //             goals:"",
        //             money_source:"",
        //             legal_location:"",
        //             legal_bank_acc:"",
        //             legal_id:4467,
        //             location:"",
        //             connection:"",
        //             certificate_number:"aJmZkWOfkJpAcsutUfRpSQpMLoIBqwHe",
        //             registration_date:"15/04/1989",
        //             final_registration_unit:"oPXTDVUFjoWiIwxIKYKMfpFyjdNxixNPzzeweQRxXLyuVnyJbjoJkrGdslqkJDIxLGiDUKduVnZrSKIqKNXzjOHymWQEPWPrPqDfJyLYVWUYOYYTblbERjwzbcGRBlJI",
        //             duplicate_doc_name:"lEwtxhxqKNEnDKmITDCtqvPoXigIYsuTHVRzWBhkuCDjDpuhDaNmkSrjQhwLBcPOcQUujyXjWQbboXcYwbVLoTpPwVjKmfaiarZLakrzXRSQOMBEJQNuagoEIjpZxVsF",
        //             duplicate_doc_number:690,
        //             duplicate_doc_date:"01/12/2003",
        //             duplicate_doc_issued:"yIzYuMsZqJuDaoNHlfVGZBGUYtckZscinrkKijwVAUVjvTaPUQmdoXIHLjcULerFYNPhmUErPQtPKuaGyzYaJsNenRNXsiqNEvTHfPvNnfYVjsCsUsBETFfZCYGHIUdZ",
        //             duplicate_date:"15/03/2013",
        //             duplicate_unit:"lIWTYkcmZOzQFGaVvlLYTPIQaubMcUmunJHHSgsOGllHKWnPzfzFBbxndXVcwewVpciUuwaBzhhljXpUgOmGCCOGgZCdXNmLMKybMiHKuXTbLJkdbKKnDebGjwdeijLU",
        //             re_registration:false,
        //             termination_date:"30/07/1971",
        //             termination_doc_name:"CcGBLSmUjfphNIHQMpFEUVOaESKgyKoCoJNnpVVxjsSIIoSbuyPjZputYypJieiNlZAZRluyqvCcDLZFjVfjXwhhamoVzcMFfNFvMFzqBkWVeKkOWQhpPBfIZqSuvmIM",
        //             termination_doc_number:8571,
        //             termination_doc_issued:"aKMLaVaitnriPMGqJUWTvQNxLUOCbsntCAZVVKdoykaeOztqIvnNlnAOWiRRPHXGrtsfAtXdIOteXeRhBmtDwCcnxRYbdpbgNpLkLuUcXElMnDZFrRPLUpmHHsptzPyI",
        //             additional:"",
        //             registrar:8185
        //         }
        //     });
        //     setDuplicateExists(entity.data.duplicate_doc_date ? true : false);
        //     setTerminated(entity.data.termination_date ? true : false)
        //     setWarning(Messages.Empty);
        // }
